...
Requirements: **Python 3.11 through 3.14**, **git**. PostgreSQL is *not* a prerequisite — QuorumGit embeds its own instance via [pg0](https://github.com/vectorize-io/pg0).
...
Python 3.11 through 3.14. Tests run against a live throwaway pg0 instance and real Git repositories — nothing is mocked. The suite covers the full acceptance workflow (register → claim → isolate → block overlap → parallel work → checkpoint → handoff → accept → approval-gated takeover → audit → restart survival), real-push hook enforcement, concurrent voting/consumption/resolution races, and a branding gate that keeps application code free of any identity other than QuorumGit.
