...
def test_open_handoff_reserves_task(conn, git_repo):
    ...
    with pytest.raises(work.ClaimRefused, match="reserved for open handoff"):
        work.claim_task(
            conn, task, c, branch="feat/other", scope_globs=["src/**"]
        )

    blocked = conn.execute(
        "SELECT count(*) FROM conflict_events WHERE task_id = ? "
...
def test_branch_frozen_during_handoff_gap(committed_conn, tmp_path, cfg):
    ...
    conn = committed_conn
    _repo, a, b, _ = _setup(
        conn, git_repo=make_git_repo(tmp_path / "seed")
    )
    seed = tmp_path / "seed"
    ...
    claim_a, _, _ = work.claim_task(
        conn, task, a, branch="feat/freeze", scope_globs=["src/**"]
    )
