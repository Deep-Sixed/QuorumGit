...
def test_doctor_preserves_changed_checkout(conn, git_repo, cfg, drift):
    ...
    path = Path(wt["path"])
    assert not any(item["worktree_id"] == wt["id"] for item in trees.doctor_worktrees(conn))
    if drift == "repository":
        subprocess.run(["git", "-C", str(git_repo), "worktree", "remove", str(path)], check=True, capture_output=True)
        make_git_repo(path)
    elif drift == "dirty":
        (path / "untracked.txt").write_text("preserve agent work", encoding="utf-8")
    else:
        args = ["checkout", "--detach"] if drift == "detached" else ["checkout", "-b", "feat/other"]
        subprocess.run(["git", "-C", str(path), *args], check=True, capture_output=True)
    work.release_claim(conn, claim, name)
    expected = {"repository": "repository_mismatch", "detached": "detached_head",
                "branch": "branch_mismatch", "dirty": "orphaned"}[drift]
    report = next(item for item in trees.doctor_worktrees(conn) if item["worktree_id"] == wt["id"])
    ...
    assert path.exists()
    if drift == "dirty":
        assert (path / "untracked.txt").read_text(encoding="utf-8") == "preserve agent work"
