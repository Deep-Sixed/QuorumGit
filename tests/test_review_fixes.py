...
def _pending_approval(conn, threshold=1):
    op = {"type": "protected_ref_update", "repository": "vote-race",
          "n": uuid.uuid4().hex}
...
def _race_votes(initialized_store, op, first_vote, second_vote):
    """First voter holds the row lock uncommitted while the second votes."""
...
def test_concurrent_no_and_yes_denial_stands(initialized_store):
    ...
    with psycopg.connect(initialized_store) as setup:
        ...
        setup.commit()
    approval, second_error = _race_votes(initialized_store, op,
                                         first_vote=False, second_vote=True)
...
def test_concurrent_yes_votes_satisfy_threshold(initialized_store):
    ...
    with psycopg.connect(initialized_store) as setup:
        ...
        setup.commit()
    approval, second_error = _race_votes(initialized_store, op,
                                         first_vote=True, second_vote=True)
...
def _approve_update(conn, repo_name, hub, clone, refname="refs/heads/main"):
    ...
    op = {"type": "protected_ref_update", "repository": repo_name,
          "refname": refname, "oldrev": oldrev, "newrev": newrev}
...
def test_approval_does_not_bypass_handoff_freeze(committed_conn, tmp_path, cfg):
    ...
    claim_a, _, _ = work.claim_task(conn, task, a, branch="main",
                                    scope_globs=["src/**"])
...
def test_refused_takeover_preserves_holder_and_approval(
    ...
    operation = {
        "type": "lease_takeover", "repository": repo_name, "task_id": task1,
    ...
    # Refused replacement: c's takeover branch collides with b's claim.
    refused = _cli(cfg, "claim", str(task1), "--takeover",
                   "--branch", "feat/two", "--scope", "src/x/**",
                   "--no-worktree", agent=c)
    ...
    assert gate.get_approval(
        conn, gate.operation_hash(operation))["status"] == "approved", \
        "refused takeover must not consume the approval"
    assert work.get_task(conn, task1)["status"] == "claimed"
    conn.commit()  # end this snapshot before the CLI writes again

    # The same approval then authorizes a valid takeover atomically.
    taken = _cli(cfg, "claim", str(task1), "--takeover",
                 "--branch", "feat/one-b", "--scope", "src/x/**",
                 "--no-worktree", agent=c)
    assert taken.returncode == 0, taken.stderr
    holder = work.active_claim_for_task(conn, task1)
    assert holder is not None and holder["agent"] == c
    assert gate.get_approval(
        conn, gate.operation_hash(operation))["status"] == "denied"  # consumed


def test_task_lock_serializes_release_with_takeover_check(
    ...
    locker = psycopg.connect(initialized_store)
    releaser = psycopg.connect(initialized_store)
    started = threading.Event()
...
def test_decline_requires_registered_addressee(conn, git_repo):
    ...
    with pytest.raises(handoff.HandoffError, match="only the addressee"):
        handoff.decline_handoff(conn, hid, a)  # creator cancels instead
...
def test_cancel_is_creator_only(conn, git_repo):
    ...
    # Cancellation ends the reservation: the task is claimable again.
    new_claim, _, _ = work.claim_task(conn, task, c, branch="feat/fresh",
                                      scope_globs=["src/**"])
    assert new_claim
...
def _race_accept_and_decline(initialized_store, git_repo, winner):
    ...
    first = psycopg.connect(initialized_store)
    second = psycopg.connect(initialized_store)
    started = threading.Event()
    finished = threading.Event()
    errors: list[Exception] = []
    ...
    try:
        ...
        assert started.wait(timeout=10)
        assert not finished.wait(timeout=0.5), \
            "losing resolution bypassed the handoff row lock"
        first.commit()
        thread.join(timeout=30)
...
def test_concurrent_accept_wins_over_decline(
    initialized_store, git_repo
):
    _race_accept_and_decline(initialized_store, git_repo, "accepted")


def test_concurrent_decline_wins_over_accept(
    initialized_store, git_repo
):
...
def _claim_with_worktree(conn, cfg, git_repo):
    ...
    claim_id, _, _ = work.claim_task(conn, task, agent, branch="feat/x",
                                     scope_globs=["src/**"])
...
def test_checkpoint_rejects_unreachable_commit(conn, cfg, git_repo):
    claim_id, agent = _claim_with_worktree(conn, cfg, git_repo)
    # A real commit on main is not reachable from the claimed branch.
    (git_repo / "unrelated.txt").write_text("elsewhere\n")
    env = {"GIT_AUTHOR_NAME": "t", "GIT_AUTHOR_EMAIL": "t@localhost",
           "GIT_COMMITTER_NAME": "t", "GIT_COMMITTER_EMAIL": "t@localhost"}
    import os
    subprocess.run(["git", "-C", str(git_repo), "add", "-A"],
                   check=True, capture_output=True)
    subprocess.run(["git", "-C", str(git_repo), "commit", "-m", "main moves"],
...
def test_handoff_rejects_nonexistent_last_commit(conn, cfg, git_repo):
    claim_id, agent = _claim_with_worktree(conn, cfg, git_repo)
    record = {"completed": "part", "remaining": "rest",
